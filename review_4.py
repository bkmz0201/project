...
class ReceiveData:
    ...
    def load(self, file):
        ...
        self.data = pd.read_csv(file, delimiter=str(delimiter))
        return self.data


class PrepareData:
    def __init__(self):
        self.data_to_prepare = None

    def filenames_rename(self, data):
        ...
        # исправить срочно
        self.data_to_prepare = data
        for index in range(len(self.data_to_prepare)):
            new_file_name = self.data_to_prepare['filename'][index]
            new_file_name = new_file_name[new_file_name.rfind('/') + 1:]
            self.data_to_prepare['filename'][index] = new_file_name
        self.data_to_prepare = self.data_to_prepare.rename(columns={'filename': 'file'})
        return self.data_to_prepare
