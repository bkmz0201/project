...
# Для более удобной работы с полученными данными их нужно объединить в один DataFrame.
...
class ReceiveSign:
    ...
    def calculation_sign(self, data1):
        for elem in data1.values:
            print(elem)
...
# Cчитывание данных из CSV файлов и преобразование в DataFrame через Pandas
def load_file(file_name):
    with open(file_name) as fp:
        delimiter = csv.Sniffer().sniff(fp.read(5000)).delimiter
    data = pd.read_csv(file_name, delimiter=str(delimiter))
...
car_data = load_file(file_1)
sign_data = load_file(file_2)
prepare = PrepareData()
